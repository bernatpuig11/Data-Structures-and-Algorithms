...
struct PLAYER_NAME : public Player {
  ...
  virtual void play () {
    ...
    //Moure una casella les treballadores a la primera ronda
    if (round() < 12) {
        ...
        for (int k = 0; k < int(perm.size()); ++k) {
            ...
            Cell c = cell(p);
            if (pos_ok(p) and c.type != Water and c.id == -1)
                move(worker_id, default_dir); 
        }
        
        VI my_soldier_ids = soldiers(me());
        default_dir = Right;
        if (me() == 1 or me() == 2) default_dir = Left;
        for (int k = 0; k < int(my_soldier_ids.size()); ++k) {
            int soldier_id = my_soldier_ids[k];
            Ant soldier = ant(soldier_id);
            Pos p = soldier.pos + default_dir;
            Cell c = cell(p);
            if (pos_ok(p) and c.type != Water and c.id == -1)
        ...
        default_dir = Right;
        if (me() == 1 or me() == 2) default_dir = Left;
        for (int k = 0; k < int(my_queen_ids.size()); ++k) {
            int queen_id = my_queen_ids[k];
            Ant queen = ant(queen_id);
            Pos p = queen.pos + default_dir;
            Cell c = cell(p);
            if (pos_ok(p) and c.type != Water and c.id == -1)
        ...
        VI perm = random_permutation(my_worker_ids.size());
        default_dir = Down;
        if (me() == 0 or me() == 1) default_dir = Up;
        for (int k = 0; k < int(perm.size()); ++k) {
            int worker_id = my_worker_ids[perm[k]];
            Ant worker = ant(worker_id);
            Pos p = worker.pos + default_dir;
        ...
        /*
        default_dir = Right;
        if (me() == 0 or me() == 3) default_dir = Left;
        for (int k = 0; k < int(perm.size()); ++k) {
            int worker_id = my_worker_ids[perm[k]];
            Ant worker = ant(worker_id);
            Pos p = worker.pos + default_dir;
        ...
        default_dir = Up;
        Dir aux = Left;
        if (me() == 0) default_dir = Down; aux = Left;
        if (me() == 1) default_dir = Down; aux = Right; 
        if (me() == 2) default_dir = Up; aux = Right;
        for (int k = 0; k < 1; ++k) {
            int soldier_id = my_soldier_ids[k];
            Ant soldier = ant(soldier_id);
            Pos p = soldier.pos + default_dir;
        ...
        int soldier_id = my_soldier_ids[2];
        Ant soldier = ant(soldier_id);
        Pos p = soldier.pos + aux;
        Cell c = cell(p);
        if (pos_ok(p) and c.type != Water and c.id == -1) 
            move(soldier_id, aux); 
        ...
        default_dir = Up; 
        aux = Right;
        if (me() == 0) default_dir = Down; aux = Right;
        if (me() == 1) default_dir = Down; aux = Left; 
        if (me() == 2) default_dir = Up; aux = Left;
        for (int k = 0; k < int(my_queen_ids.size()); ++k) {
            int queen_id = my_queen_ids[k];
            Ant queen = ant(queen_id);
            Pos p = queen.pos + default_dir;
            Cell c = cell(p);
            if (pos_ok(p) and c.type != Water and c.id == -1 and round() < 15)
                move(queen_id, aux); 
            else if (pos_ok(p) and c.type != Water and c.id == -1)
                move(queen_id, default_dir); 
        }
    }
