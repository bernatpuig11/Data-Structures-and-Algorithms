...
struct PLAYER_NAME : public Player {
    ...
    */
        ...
        if (cell(p).id != -1) move = false;
            ...
            for (int i = -1; i < 2; ++i) {
                for (int j = -1; j < 2; ++j) {
                    ...
                    else if (i == 0 and j == 0) if (cell(p).type == Water) move = false;
                }
            }
        }
        return move;
    ...
    bool soldier_enemy_circle(Pos p, int& i, int&j) {
        for (i = -2; i < 3; ++i) {
            for (j = -2; j < 3; ++j) {
                Pos n = Pos(p.i + i, p.j + j);
                if (pos_ok(n) and cell(n).id != -1) {
    ...
    bool worker_soldier_enemy_near(Pos p, int& d) {
        for (int i = -1; i < 2; ++i) {
            for (int j = -1; j < 2; ++j) {
                if ((i == -1 and j == 0) or (i == 0 and j == -1) or (i == 0 and j == 1) or (i == 1 and j == 0)) {
                    Pos n = Pos(p.i + i, p.j + j);
                    if (pos_ok(n) and cell(n).id != -1) {
                        Ant a = ant(cell(n).id);
                        if ((a.type == Soldier or a.type == Worker) and a.player != me()) {
    ...
    bool choose_soldier() {
        if (round() < 100 and soldiers(me()).size() < 3 and workers(me()).size() > 4 and winners()) return true;
        return false;
    }
    ...
    void move_queen() {
        ...
        int d = -1;
        if (round() % 2 == 0) {
            if (worker_soldier_enemy_near(queen.pos, d)) {
                ...
                if (d != -1) {
                    ...
                    if (queen_enemy_near(queen.pos + default_dir)) {
        ...
        else if (queen.reserve[0] > 0 and queen.reserve[1] > 0 and queen.reserve[2] > 0) {
            d = calc_dir_lay(queen.pos);
            if (d == 0) default_dir = Down;
            else if (d == 1) default_dir = Right;
            else if (d == 2) default_dir = Up;
            ...
            else if (choose_soldier()) {
                if (queen.reserve[0] > 2 and queen.reserve[1] > 2 and queen.reserve[2] > 2) {
                    lay(queen_id, default_dir, Soldier);
                }
            }
            else if (not soldier_enemy_circle(queen.pos, i, j) or round() > 200) lay(queen_id, default_dir, Worker);
        }
    } 
    ...
    void move_workers() {
        ...
        for (int k = 0; k < int(perm.size()); ++k) {
            ...
            int i, j, d;
            if (soldier_enemy_circle(worker.pos, i, j)) {
                bool moved = false;
                if (i == 0) {
                    if (j < 0) {
                        default_dir = Right;
                        if (can_move(worker.pos + default_dir) and pos_ok(worker.pos + default_dir)) moved = true;
                    }
                    else {
                        default_dir = Left;
                        if (can_move(worker.pos + default_dir) and pos_ok(worker.pos + default_dir)) moved  = true;
                ...
                if (moved) {
                    ...
                    Pos n = worker.pos + default_dir;
                    if (pos_ok(n + default_dir + default_dir) and cell(n + default_dir + default_dir).id != -1) {
                        Ant a = ant(cell(n + default_dir + default_dir).id);
                        if (a.type == Soldier and a.player != me()) b = false;
                    }
                ...
                if (worker.bonus == None and cell(worker.pos).bonus == None) {
                    ...
                    if (d != -1) {
                        ...
                        else if (can_move(worker.pos + default_dir)) {
                            bool b = true;
                            for (int i = 0; i < pos_disp.size(); ++i) {
                                if (pos_disp[i] == worker.pos + default_dir) b = false;
                            }
    ...
    */
    virtual void play () {
        ...
        if (round() == 140) soldier_done = false;
